...
def settings_api_view_create(request):
    if request.method == 'POST':
        ...
        field_add = {"event_id": get_event_id(),
...
@api_view(['POST',])
def scale_response_view_submit(request):
    if request.method == 'POST':
        response = request.data
        id = response['id']
        field_add = {"_id": id}
        ...
        if response['instance_id'] > number_of_scale:
            return Response(status=status.HTTP_400_BAD_REQUEST)

        field_add = {"event_id": get_event_id(), "scale_data": {"scale_id": id, "scale_type": "nps scale"},
