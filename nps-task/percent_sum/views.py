    ...
    if request.session.get("userinfo"):
        ...
        if request.method == 'POST':
            ...
            try:
                ...
                #return redirect(f"http://127.0.0.1:8000/percent_sum/percent_sum-scale1/{template_name}")
                return redirect(f"https://100035.pythonanywhere.com/percent_sum/percent_sum-scale1/{template_name}")
            except:
...
def dowell_scale1(request, tname1):
   ...
   #print(data)
    ...
    if request.method == 'POST':
        ...
        try:
            field_add={"scale_name":context["scale_name"], "scale_type":"percent_sum"}
            response=dowellconnection("dowellscale","bangalore","dowellscale","scale_reports","scale_reports","1094","ABCDE","fetch",field_add,"nil")
            data=json.loads(response)
            x = data["data"]
            for i in x:
...
def default_scale_admin(request):
    if request.session.get("userinfo"):
        ...
        context["percent_sum_all"] = sorted(data["data"], key=lambda d: d['_id'], reverse=True)

        return render(request, 'percent_sum/default.html', context)


def rolescreen(request):
    return render(request, 'percent_sum/landing_page.html')
    ...
    try:
        if user["username"]:
            if user["role"]=='Client_Admin' or user["role"]=='TeamMember':
                response = redirect("percent_sum:default_page_admin")
                response.set_cookie('user', user['username'])
                return response
            else:
                response = redirect("percent_sum:default_page")
                response.set_cookie('user', user["username"])
                return response
    except:
        return redirect('https://100014.pythonanywhere.com/')
