        ...
        <div className={`h-full w-full lg:w-3/12 border overflow-y-auto`}>
                <h2 className='flex items-center gap-2 p-2 font-medium'>
                    ...
                    {scaleData && scaleData?.map((scale, index)=>(
                        <>
                            <Button width={'full'} onClick={()=>navigateTo(`/100035-DowellScale-Function/percent-sum-scale-settings/${scale._id}`)} key={index}>{scale?.settings?.name}</Button>
                        </>
                    ))}

                </div>
                <div className='stage h-full w-full lg:w-5/12 border flex-1  p-2'>
                <h1 style={{textAlign:'center',marginTop:"20%"}}>Percent Sum Scale</h1>             
                ...
                <input type="range" min="1" max={100-sliderValue} onChange={e=>setSliderValue2(e.target.value)} className="slider" id="myRange"/>
                <h4 style={{textAlign:"center"}}>{sliderValue2}%</h4>
                </div>
                
                    <div className='flex items-center justify-end w-full my-4'>
                        <Button primary width={'3/4'} onClick={()=>navigateTo(`/100035-DowellScale-Function/create-percent-sum-scale`)}>create new scale</Button>
                    </div>
                </div>
            </div>
...
export default PercentSumScale