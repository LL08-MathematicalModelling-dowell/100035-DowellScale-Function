...
const UpdatePMSSettings = ({ handleToggleUpdateModal }) => {
  ...
  const fetchScales = async () => {
    try {
      ...
      const response = await axios.get(
        `https://100035.pythonanywhere.com/perceptual-mapping/perceptual-mapping-settings/?scale_id=${slug}`
        // requestOptions
      );
      const results = await response.data;
      setData(results.success.data.settings);
      // console.log(results.success.data.settings.name)
      // setIsLoading(false);
      console.log(results)
    } catch (error) {
  ...
  useEffect(() => {
    if (data) {
      ...
      no_of_scale: data.no_of_scales,
      allow_resp: data.allow_resp,
      X_upper_limit: data.X_upper_limit,
      Y_upper_limit: data.Y_upper_limit,
      X_left: data.X_left,
  ...
  const handleSubmit = async (e) => {
    ...
    var requestOptions = {
      ...
      item_count: formData.item_list?.lenghth,
...
console.log(requestOptions)
      ...
      if (result.error) {
        ...
        const timeout = setTimeout(
          ...
          window.location.reload("/")
            // navigate(
            //   `/100035-DowellScale-Function/single-perceptual-scale-settings/${slug}`)
            },2000
        );
  ...
  return (
    <div className="fixed top-0 left-0 flex justify-center w-full h-screen bg-primary/40" style={{width:"70%",overflow:"auto",marginLeft:"15%"}}>
    {/* <div className="relative p-5 m-auto bg-white border" style={{width:"73%"}}> */}
      ...
      <form
        className="lg:w-[100%] w-full  mx-auto border-4 border-gray-500 bg-[#d9edf7] shadow-md p-8" style={{height:"60em"}}
