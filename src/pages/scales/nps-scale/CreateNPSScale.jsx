  ...
  const handleSubmitNPSScale = async()=>{
    ...
    try {
        ...
        if(response.status===201){
            ...
            // },2000)
            navigateTo(`/100035-DowellScale-Function/nps-scale-settings/${response?.data?.data?.scale_id}`)
        }
    } catch (error) {
        console.log(error);
    ...
    <div className='flex flex-col items-center justify-center w-full h-screen font-Montserrat'>
      <div className='w-full p-5 border md:w-7/12'>
        <div className='flex justify-between'>
            <h2 className="mb-3 text-sm font-medium text-center capitalize">set up your Ranking</h2>
            {timeOn && (
